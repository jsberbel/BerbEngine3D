...
void SinglePlayerScreen::Build(void) {
	...
	int nw = (WINDOW.screenHeight * FIXED_ASPECT_RATIO);
	m_camera.Resize(nw + (WINDOW.screenWidth - nw) / 2, WINDOW.screenHeight);
	m_minimapCamera.Resize(nw + (WINDOW.screenWidth - nw) / 2, WINDOW.screenHeight);
	...
	m_renderer.InitFramebuffer(WINDOW.screenWidth, WINDOW.screenHeight);
}
...
void SinglePlayerScreen::OnEntry(void) {
	...
	m_player.body.meshRef = &ASSET.FindMesh("mesh_kart_default");
	m_player.body.materialRef = &ASSET.FindMaterial("material_kart_default");
	...
	skybox.meshRef = &ASSET.FindMesh("mesh_skybox");
	skybox.materialRef = &ASSET.FindMaterial("material_skybox");
	m_renderer.AddObject(&skybox);

	circuit.transform.scale = { 1, 1, 0.2 };
	circuit.meshRef = &ASSET.FindMesh("mesh_circuit");
	circuit.materialRef = &ASSET.FindMaterial("material_circuit");
	m_renderer.AddObject(&circuit);

	debugCollisions.color = { 0,0.5, 0 };
	debugCollisions.meshRef = &ASSET.FindMesh("mesh_debug_collisions");
	...
	for (int i = 0; i < MAX_AI_ENEMIES; ++i) {
		...
		m_aiEnemies[i].body.meshRef = &ASSET.FindMesh("mesh_kart_default");
		m_aiEnemies[i].body.materialRef = &ASSET.FindMaterial("material_kart_0" + std::to_string(i));
	...
	itemBox.meshRef = &ASSET.FindMesh("mesh_item_box");
	itemBox.materialRef = &ASSET.FindMaterial("material_item_box");
...
PowerUp *SinglePlayerScreen::GetRandPowerUp(int ID) {
    ...
    switch (0) { // Get random number for MAX_POWERUPS powerups
        case 3: { // BULLET BILL
            ...
            temp->meshRef = &ASSET.FindMesh("mesh_bullet_bill"); // Assign mesh
            temp->materialRef = &ASSET.FindMaterial("material_bullet_bill"); // Assign material
            if (ID == -1) {
            ...
            temp->meshRef = &ASSET.FindMesh("mesh_banana"); // Assign mesh
            temp->materialRef = &ASSET.FindMaterial("material_banana"); // Assign material
            ...
            temp->meshRef = &ASSET.FindMesh("mesh_shell"); // Assign mesh
            temp->materialRef = &ASSET.FindMaterial("material_red_shell"); // Assign material
            ...
            temp->meshRef = &ASSET.FindMesh("mesh_shell"); // Assign mesh
            temp->materialRef = &ASSET.FindMaterial("material_green_shell"); // Assign material
...
void SinglePlayerScreen::Update(void) {
	...
	if (m_player.stunned) m_player.body.transform.rotation.y = float((clock()) % 360);
		...
		m_carPhysics.Update(temp, TIME.deltaTime, colVecAi);
	...
	// IA update
	m_aiPhysics.Update(TIME.deltaTime);
	for (int i = 0; i < MAX_AI_ENEMIES; ++i) {
	...
	if (clock() > itemBox.activeCounter + POWERUP_SPAWN_DELAY) { // Check when item box spawns
		...
		itemBox.transform.position.y = 3.0f + sin(clock()*0.01f)*30.0f*TIME.deltaTime; // Update sinoidal Y movement
	...
	if (m_player.powerUp != nullptr && m_player.powerUp->enabled) {
		m_player.powerUp->Update(TIME.deltaTime);// Update player power up if exists
		for (int i = 0; i < MAX_AI_ENEMIES; ++i) {
	...
	for (int i = 0; i < MAX_AI_ENEMIES; ++i) 
		if (m_aiEnemies[i].powerUp != nullptr && m_aiEnemies[i].powerUp->enabled) {
		m_aiEnemies[i].powerUp->Update(TIME.deltaTime); // Update player power up if exists
...
void SinglePlayerScreen::CheckInput(void) {
		...
		if (evnt.type == SDL_WINDOWEVENT) {
			switch (evnt.window.event) {
				case SDL_WINDOWEVENT_RESIZED:
					SDL_GetWindowSize(WINDOW.SDLWindow, &WINDOW.screenWidth, &WINDOW.screenHeight);
					glViewport(0, 0, WINDOW.screenWidth, WINDOW.screenHeight); // Set the OpenGL viewport to window dimensions
					int nw = (WINDOW.screenHeight * FIXED_ASPECT_RATIO);
					m_camera.Resize(nw + (WINDOW.screenWidth - nw) / 2, WINDOW.screenHeight);
					break;
			}
	...
	if (INPUT.isKeyPressed(SDLK_e)) RendererList::DEBUG_DRAW = !RendererList::DEBUG_DRAW;
	if (INPUT.isKeyPressed(SDLK_q)) RendererList::WIREFRAME_MODE = !RendererList::WIREFRAME_MODE;
	if (INPUT.isKeyPressed(SDLK_SPACE)) // Activate player powerup
		if (m_player.powerUp != nullptr) m_player.powerUp->Activate(), m_player.itemSlot.SetImage("images/slot_empty.jpg");
	if (INPUT.isKeyPressed(SDLK_ESCAPE)) SCENE.SetScreen(SCREEN_MENU); // Exit game
}
